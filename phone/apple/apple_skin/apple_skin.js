...
function getCurrentProduct() {
  ...
  // Use image as a "variant" identifier for design selection.
  // Include model so same design can be ordered for different models.
  const id = `${brand}:${title}:${image}:${model || "NO_MODEL"}`;

  return { id, title, brand, price, image, model };
}

function requireModelOrWarn() {
  const modelInput = document.getElementById("modelInput");
  const model = (modelInput?.value || "").trim();
  if (model.length < 3) {
    alert("Please enter your phone model (example: iPhone 15 Pro Max) before adding to cart.");
    modelInput?.focus();
    return null;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  const savedModel = (localStorage.getItem(MODEL_STORAGE_KEY) || "").trim();
  const modelInput = document.getElementById("modelInput");
